...
class SeparateHead(nn.Module):
    ...
    def forward(self, x):
        ret_dict = self.forward_cls_preds(x)
        ret_dict.update(self.forward_remaining_preds(x))
        return ret_dict
    ...
    def forward_remaining_preds(self, spatial_features_2d):
        ...
        for cur_name in self.sep_head_dict:
            if cur_name != 'hm':
                d[cur_name] = self.__getattr__(cur_name)(spatial_features_2d)
        return d


class CenterHeadMultiImprecise(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        for i in range(len(input_channels)):
            ...
            self.rpn_head_alternatives.append(heads_list)
    ...
    def forward_cls_preds(self, data_dict):
        ...
        if 'heads_to_run' not in data_dict:
            for rpn_head in self.rpn_head_alternatives[cur_stg-1]:
                ...
                pred_dicts.append(rpn_head.forward_cls_preds(spatial_features_2d))
        else:
            for h in data_dict['heads_to_run']:
                rpn_head = self.rpn_head_alternatives[cur_stg-1][h]
                pred_dicts.append(rpn_head.forward_cls_preds(spatial_features_2d))
        data_dict['pred_dicts'] = pred_dicts
        data_dict['cls_preds'] = [ret_dict['cls_preds'] for ret_dict in pred_dicts]
    ...
    def forward_remaining_preds(self, data_dict):
        ...
        for i, h in enumerate(data_dict['heads_to_run']):
            rpn_head = self.rpn_head_alternatives[cur_stg-1][h]
            pred_dicts[i].update(rpn_head.forward_remaining_preds(
                data_dict[f'shared_conv_out_{cur_stg}']))
...
#        if not self.training or (self.predict_boxes_when_training and cur_stg==3):
#            pred_dicts = self.generate_predicted_boxes(
#                data_dict['batch_size'], pred_dicts
#            )
#
#            if self.predict_boxes_when_training and cur_stg==3:
